$(function(){
	...
	$("#ccc").keyup(function(){
		if(!isAccountComplete()){
			...
			$("button#calc").attr('disabled',"disabled");
			$("#iban").html("");
			$("#bic").html("");
		} else {
			$("button#calc").removeAttr('disabled');
			$("button#calc").attr('enabled');
		}
...
var generateIban = function(ccc){
	ccc = ccc.split("-").join("");
	var iban = ccc;
	var countryCode = "ES"
	var countryAsNumber = "142800"
	var controlDigits = "";

	var auxIban = ccc + countryAsNumber;
	controlDigits = 98 - modulo(auxIban, 97);
	if(controlDigits < 10)
		controlDigits = "0" + controlDigits;

	iban = countryCode + controlDigits + "" + ccc;

	return iban.match(/..../g).join(" ");
}

var updateBic = function(value){
	ccc = value.split("-");
	bankCode = ccc[0];
	var bic = getBic(bankCode);
	if(bic == undefined)
		bic = "";
	$("#swift").val(bic);
}
